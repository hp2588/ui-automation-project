...
public class MapFacility_Providing extends BaseTest {
	...
	public void getOrganizationResourceFacilityID() throws ParseException {
		...
		String facilityName = TestDataPropertyReaderAndWriter.getProperty("FacilityNameProviding");
	...
	public void PostUserFacilityMapping() throws JSONException {
		...
		Response response = given().auth().oauth2(TestDataPropertyReaderAndWriter.getProperty("access_token"))
				.contentType("application/json\r\n").header("tenantCode", getData("IDM.tenantCode"))
				.header("client_id", getData("IDM.client_id")).log().body().body(obj.toString()).post();

		if (response.getStatusCode() == 201) {
			System.out.println("Facility is mapped successfully to user " + getData("Auth.userNameAdminUser").trim());
