...
public class Story_1064317 extends BaseTest{
	String orderName;
	...
	public void Test01_1121818(Method method)  {
		...
		test.remoteWebOrderActions.verifyButtonIsDisabled("ReceivedandSent");
	} 

	@Test(priority = 2, description = "VPLX: Remote ordering:[UI]-When user create a Remote order with order quantity equal to package size in case of IsSplit True, then only PO is created in TQ")
	public void Test02_1092544(Method method)  {
		...
		*/
		...
		test.siteConfigurationAction.verifyNewlyAddedPrinterNameInPrinterList(ItemCode2);

		// TODO
		test.supportDataActions.enterSearchTermInSearchField(ItemCode2, "search");
		test.supportDataActions.clickRecordNameToEdit(ItemCode2);
		...
		test.remoteWebOrderActions.selectDropdownForRO("selectDestination",FacilityName+" - "+DestinationName);
		test.remoteWebOrderActions.clickButton("buildNewOrder");
		String OrderName_Split = test.remoteWebOrderActions.EnterRandomValueInInputFieldOnAddNewRemoteOrder("orderNameInput",
