...
Scenario:
	...
	* def labelCategory = dbUtils1.readRow("exec sp_set_session_context @key=N'TenantKey', @value='" + tenantKey + "';SELECT LabelCategoryKey, LabelCategoryCode FROM LabelCategory WHERE LabelCategoryCode='binlabel'")
	* def transactionPriority = dbUtils2.readValue("exec sp_set_session_context @key=N'TenantKey', @value='" + tenantKey + "';SELECT TransactionPriorityKey FROM TransactionPriority WHERE facilitykey = '" + facilityKey + "' AND TRANSACTIONPRIORITYCODE = 'STAT';")
