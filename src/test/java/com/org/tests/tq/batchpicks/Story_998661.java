...
public class Story_998661 extends BaseTest {
	...
	@Test(priority = 12, description = "VPLX: Batch Picks :[UI]- User is able to see "
			+ "Batch transaction in TQ[UI] when the Batch is Processed "
			+ "from 'Review/Release Batch Pick' screen.	Kumar Prakash")
	...
	@Test(priority = 13, description = "VPLX: Batch Picks :[UI]- System is removed Ignore/Release button "
			+ "once the whole Batch processed.")
	public void Test13_1053278(Method method) {
		ExtentTestManager.startTest(method.getName(),
				"VPLX: Batch Picks :[UI]- System is removed Ignore/Release button once the whole Batch processed.");
		test.transactionQueueActions.verifyActionButtonAndClick();
		test.transactionQueueActions.verifyActionItemsAndClick(getData("ActionsItemsList.Item8"));
		test.transactionQueueActions.headingBatchPick();
	...
	@Test(priority = 14, description = "VPLX:Batch Picks:[UI]- Accept button is disabled "
			+ "if sum of Quantity to Process is not equal to Picked Quantity.")
	public void Test14_1054840_1054782_1055015_1059447_1059451(Method method) {
		ExtentTestManager.startTest(method.getName(),
				"VPLX:Batch Picks:[UI]- Accept button is disabled if sum of Quantity to Process is not equal to Picked Quantity.");
		 ...
		 * verifyBatchPopupSubHeading("Item ID",
		 * TestDataPropertyReaderAndWriter.getProperty("ItemID")));
		 * Assert.assertTrue(test.transactionQueueActions.
		 * verifyBatchPopupSubHeading("Item Description",
		 * TestDataPropertyReaderAndWriter.getProperty("ItemName"))); Assert.assertTrue(
		 * test.transactionQueueActions.
		 * verifyBatchPopupSubHeadingQuantity("Requested Quantity", activeQuantity));
		 * Assert.assertTrue( test.transactionQueueActions.
		 * verifyBatchPopupSubHeadingQuantity("Picked Quantity", "100"));
		 * Assert.assertTrue(test.transactionQueueActions.
		 * verifyBatchPopupSubHeadingQuantity("Correction Needed", "0"));
		 */
	...
	@Test(priority = 15, description = "VPLX:Batch Picks:[UI]-User is able to change the quantity "
			+ "when transaction is active.")
	public void Test15_1054722_1054795_1054820_1054995(Method method) {
		ExtentTestManager.startTest(method.getName(),
				"VPLX:Batch Picks:[UI]-User is able to change the quantity when transaction is active.");
		activeQuantity = test.transactionQueueActions.getCurrentQuantity();
		test.transactionQueueActions.clickOnQuantity("100");
		test.siteConfigurationAction.clickPickRoutingRuleButton("primary");
	...
	public void Test16_1053279(Method method) {
		ExtentTestManager.startTest(method.getName(),
				"VPLX: Batch Picks :[UI]-  system is removed Batch Transaction form TQ UI screen when batch pick transaction has been completed from UI.");
	...
	@Test(priority = 17, description = "VPLX:Wasting items in Transaction Queue:[UI]: User is able to Waste Item for Batch Type Transaction from Tq.")
	public void Test17_1161608(Method method) {
		ExtentTestManager.startTest(method.getName(),
				"VPLX:Wasting items in Transaction Queue:[UI]: User is able to Waste Item for Batch Type Transaction from Tq.");
		...
		test.transactionQueueActions.EnterValueInInputFieldOnWasteItemPopup("wasteQuantity","10");
