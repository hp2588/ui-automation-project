...
Scenario:
	...
	* def destinationCode = facilityCode
	* def descriptionText = destinationName
	
	Given headers req_headers
		And path destinationPostPath, facilityKey
		And def bodyOfRequest = read('classpath:com/bd/data/api/packagesharejson/destination.json')
		...
		And set bodyOfRequest.packageSharingFacilityKey = packageSharingFacilityKey
